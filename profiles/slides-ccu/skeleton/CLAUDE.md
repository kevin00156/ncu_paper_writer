...
本專案啟用 `ccu-slides-writer` skill。Claude 會依照 skill 中的口試簡報撰寫指引協助你：
...
- Windows：`..\scripts\build-slides.ps1 slides.md --profile slides-ccu`
- Linux / macOS：`../scripts/build-slides.sh slides.md --profile slides-ccu`
...
- 預設色票：中正藍 `#1A3668`（主色）+ 機械工程學系橘 `#E97A1F`（輔色）
- 若拿到中正大學 CIS 手冊或系徽向量檔的官方 RGB，把 `theme.css` 的 `--ccu-primary` / `--ccu-accent` 替換即可
